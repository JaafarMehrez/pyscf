...
def write_uhf_integrals_fort55(uhf_obj,mol,tol=1e-15):
    '''Integrals written to disk in fort.55 file format, intended for interfacing with MRCC'''
    orbs  = uhf_obj.mo_coeff
    nmo   = orbs[0].shape[0]
    nelec = mol.nelec
    eri_aaaa = ao2mo.restore(4,ao2mo.incore.general(uhf_obj._eri, (orbs[0],orbs[0],orbs[0],orbs[0]), compact=False),nmo)
    eri_bbbb = ao2mo.restore(4,ao2mo.incore.general(uhf_obj._eri, (orbs[1],orbs[1],orbs[1],orbs[1]), compact=False),nmo)
    eri_aabb = ao2mo.restore(4,ao2mo.incore.general(uhf_obj._eri, (orbs[0],orbs[0],orbs[1],orbs[1]), compact=False),nmo)
    h_core = uhf_obj.get_hcore(mol)
    ...
    mol.orbsym = symm.label_orb_symm(mol,mol.irrep_name,mol.symm_orb,orbs[0])
